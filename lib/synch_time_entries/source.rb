...
module SynchTimeEntries
	class Source
		...
		# Obtiene todos los proyectos archivados del origen por identificador
		def self.get_filed_projects_identifier(offset = 0)
			projects = []
			total = 1
			while (offset < total)
				res = redmine_request(get_endpoint('projects'), 'get', {:offset => offset, :status => 9})
